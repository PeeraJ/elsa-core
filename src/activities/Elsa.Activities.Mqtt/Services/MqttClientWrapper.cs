...
namespace Elsa.Activities.Mqtt.Services
{
    public class MqttClientWrapper : IMqttClientWrapper
    {
        ...
        private Func<MqttApplicationMessage, Task>? _messageHandler { get; set; }

        public IMqttClient Client { get; }
        public MqttClientOptions Options { get; }


        public MqttClientWrapper(IMqttClient client, MqttClientOptions options, ILogger logger)
        {
            Client = client;
            Options = options;
            _logger = logger;
        ...
        {
            if (!Client.IsConnected) await Client.ConnectAsync(Options.GenerateMqttClientCredentials()); 
        }
